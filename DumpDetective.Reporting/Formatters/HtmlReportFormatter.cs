...
namespace DumpDetective.Reporting.Formatters
{
    internal static partial class ReportFormatter
    {
        private static string ToHtml(string detailedReport, IReadOnlyList<string> insights, string dumpPath, IReadOnlyList<InsightFinding> findings)
        {
            ...
            b.AppendLine("""
                <header class="topbar">
                  <span class="topbar-logo">ðŸ•µï¸ DumpDetective</span>
            ...
            b.AppendLine($$"""
                <div class="meta-card">
                  <div class="meta-row"><span class="meta-key">ðŸ“ Dump File</span><code class="meta-val">{{HtmlEnc(dumpPath)}}</code></div>
                  <div class="meta-row"><span class="meta-key">ðŸ• Generated (UTC)</span><code class="meta-val">{{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}}</code></div>
                </div>
                """);

            b.AppendLine($$"""
                <div class="sev-bar">
                  <div class="sev-pill sev-crit">ðŸ”´ <strong>{{critCount}}</strong> Critical</div>
                  <div class="sev-pill sev-warn">ðŸŸ  <strong>{{warnCount}}</strong> Warning</div>
                  <div class="sev-pill sev-info">ðŸ”µ <strong>{{infoCount}}</strong> Info</div>
                </div>
                """);

            b.AppendLine("<section>");
            b.AppendLine("<h2>ðŸ” Insights</h2>");
            ...
            b.AppendLine("<section>");
            b.AppendLine("<h2>ðŸš¨ Findings</h2>");
            if (findings.Count == 0)
            {
            ...
            b.AppendLine("<section>");
            b.AppendLine("<h2>ðŸ“Š Detailed Analysis</h2>");
            ...
            foreach (var block in parsed.Blocks)
            {
                if (block.Label != null)
                {
                    ...
                    b.AppendLine($"  <span class=\"snapshot-hdr-title\">ðŸ“¸ Snapshot {HtmlEnc(block.Label)}</span>");
                    if (isLast)
        ...
        private static void AppendHtmlTrendSection(StringBuilder b, ReportSection trend)
        {
            ...
            b.AppendLine("<h2>ðŸ“ˆ Trend Comparison</h2>");
            ...
            {
                b.AppendLine("<h3 class=\"sub-h3\">ðŸ“Š Metric Timeline</h3>");
            ...
            {
                b.AppendLine("<h3 class=\"sub-h3\">ðŸ”º New Findings</h3>");
            ...
            {
                b.AppendLine("<h3 class=\"sub-h3\">âœ… Resolved Findings</h3>");
                b.AppendLine("<ul class=\"trend-list\">");
