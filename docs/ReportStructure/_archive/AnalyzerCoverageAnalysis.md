...
| ?? | Requires splitting an existing analyzer |
...
| Memory leak likelihood score | ?? | `RetentionAnalyzer` + `StaticRootLeakDetector` — signals exist but no unified score |
...
| Sub-item | Status | Analyzer(s) |
|----------|--------|-------------|
| Ranked suspicious types by size / count | ?? | `RetentionAnalyzer` — `TopHighlyReferencedObjects`; `StaticRootLeakDetector` — `TopRootsByRetainedBytes` |
...
| Finalizer queue backup | ? | `RetentionAnalyzer` — `FinalizerQueueCount` + `TopFinalizerTypes` |
...
**Verdict**: §11 is now substantially covered by `StringAnalyzer`. String data has been split out of `RetentionDomainResult` into a standalone `StringDomainResult`. Remaining gaps are the string-length histogram and ASCII-in-UTF-16 encoding waste heuristic.
...
| `SkippedReferenceAddresses` | ? | `RetentionAnalyzer` — `SkippedReferenceAddresses` |
...
| Sub-item | Status | Analyzer(s) |
|----------|--------|-------------|
| Finalizer queue depth (total count) | ? | `RetentionAnalyzer` — `FinalizerQueueCount` |
| Top types in finalizer queue | ? | `RetentionAnalyzer` — `TopFinalizerTypes` |
...
| 1 | `RetentionAnalyzer` | [RetentionAnalyzer.md](Analyzers/RetentionAnalyzer.md) | ?? Split — extract `StringAnalyzer`; add `SuspicionScore` | ? **Pending** |
...
| 2 | `HangAnalyzer` | [HangAnalyzer.md](Analyzers/HangAnalyzer.md) | ?? Split — extract `AsyncTaskAnalyzer` | ? **Completed** |
...
|----------|------|------|--------|----------------|--------|
| 1 | `StringAnalyzer` split from `RetentionAnalyzer` | ?? Split | Low | ? `IsStringType` flag ready | ? **Completed** |
| 2 | `AsyncTaskAnalyzer` split from `HangAnalyzer` | ?? Split | Medium | ? `TaskIndex.bin` written | ? **Completed** |
