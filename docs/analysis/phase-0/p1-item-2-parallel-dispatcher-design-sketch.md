...
1. Run `HeapIndexScanDispatcherPerfTests.cs` against a real 10GB+ dump (`DD_BENCHMARK_DUMP`) to get the sequential single-pass numbers the roadmap doc flags as still missing. If sequential already wins or is close, stop here — this redesign isn't worth its risk.
2. If parallel read-side throughput is clearly needed, prototype B against one participant (e.g. `AsyncTaskAnalyzer`, since its cap-trimming merge logic is the simplest of the four) before touching `IHeapIndexScanParticipant` for all nine.
3. Only then extend to `CrashAnalyzer`/`HangAnalyzer`/`CollectionAnalyzer`, and only then consider deleting their now-redundant `Parallel.ForEach` fallbacks in favor of the dispatcher's own parallel path plus a thin adapter that wraps `heap.Segments` as an entry source for the no-index case.
