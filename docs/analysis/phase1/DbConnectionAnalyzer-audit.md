...
| R8 | ✅ **DONE** — New `SqlTransactionAnalyzer` scans `SqlTransaction`/`IDbTransaction` objects (Active/Disposed/Other by `_connection` field liveness); `InsightEngine.DetectLongHeldTransactionOnOpenConnection` correlates Active transactions against `DbConnectionDomainResult.TopOpenConnections` addresses | Evolution | Medium — surfaces long-held transaction anti-pattern | Medium | High | **P2** |
| R9 | ✅ **DONE** — New `SqlCommandAnalyzer` scans `SqlCommand`/`IDbCommand` objects (same namespace-prefix matching), reporting Active (still references a connection) vs Detached counts per type; providers don't reliably null the connection field on `Dispose()`, so this is documented as "outstanding/wired to a connection" rather than strict disposed state | Evolution | Medium — extends coverage to full resource lifecycle | Low | High | **P2** |
| R10 | ✅ **DONE (partial, hard-need basis)** — `LiveHeapSnapshotFixture` (self-attach via `DataTarget.CreateSnapshotAndAttach`, no external .dmp) added real field-read coverage for `DbConnectionAnalyzer` and `SqlCommandAnalyzer`. `SqlTransactionAnalyzer`/`SqlConnectionPoolAnalyzer` intentionally left uncovered this way — both need a real/open DB connection to construct, which wasn't pursued speculatively; still only covered by the real-dump discrepancy tests | Improvement | Medium — replaces hard-coded dump path test | Medium | High | **P2** |
| R11 | ✅ **DONE** — New `SqlConnectionPoolAnalyzer` reads `DbConnectionPool._totalObjects`/`_connectionPoolGroupOptions._maxPoolSize`/`_minPoolSize` directly (confirmed field names by decompiling `Microsoft.Data.SqlClient`/`System.Data.SqlClient`; both share the identical `System.Data.ProviderBase.DbConnectionPool`/`Microsoft.Data.ProviderBase.DbConnectionPool` implementation — no per-provider subclass to prefix-match). Exact current-size/max-pool-size per pool, redacted connection string via the 3-hop `_connectionPoolGroup → _connectionOptions → _usersConnectionString` field chain. Scoped to the SqlClient family only — Npgsql/MySql/Oracle/Sqlite use unrelated internal pooling designs, documented as out of scope | Evolution | High — direct pool-exhaustion evidence | High | Medium | **P3** |
| R12 | ✅ **DONE** — `DbConnectionAnalyzer` now implements `IRequiresReachableGraphIndex` and runs `RootPathFinder` (same pattern as EventLeakAnalyzer/TimerLeakAnalyzer/StaticRootLeakDetector). Scoped to Gen2 (likely-leaked) open connections only, ranked by retained bytes, capped at `MaxRootPathEnrichment = 20` searches — running root-path search against the full (uncapped, potentially thousands) open-connection population wasn't viable. Root path surfaces in the new "Retention paths (Gen2 open connections)" section table and in the Gen2 finding's evidence text for the single largest offender | Evolution | High — matches SOS investigative workflow | High | Medium | **P3** |
