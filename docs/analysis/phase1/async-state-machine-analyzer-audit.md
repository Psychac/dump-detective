...
| P2-4 | Verify regex `<(.+?)>d__\d+$` against generic async state machine type names emitted by ClrMD; add `Contains(">d__")` post-processing to handle trailing generic params if needed | Improvement | Medium — correctness risk for generic async methods | Low | Medium | DONE |
