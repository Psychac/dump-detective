...
| # | Recommendation | Impact | Difficulty | Confidence | Class |
|---|---|---|---|---|---|
| P1-1 | **Add `Nullable<T>` sub-totals** (`NullableBoxedCount`, `NullableBoxedBytes`) by testing `typeName.StartsWith("System.Nullable<")` during the scan loop. Add dedicated finding in `BoxingFindingGenerator`. | High — most common boxing source in modern C# | Low | High | Improvement |
| P1-2 | **Store actual `TypeScanCap` value in `BoxingDomainResult`** and use it in all user-facing warning strings. Eliminates hardcoded "10 000" in `BoxingSectionBuilder` and `BoxingFindingGenerator`. | Medium — correctness + UX | Low | High | Improvement |
| P1-3 | **Add aggregate padding waste** (`sum(WastedPaddingBytes * BoxCount)`) to `BoxingDomainResult` and the section builder. Engineers need the heap-scale impact, not just the per-type ratio. | High — makes padding actionable at scale | Low | High | Improvement |
| P1-4 | **Remove dead `typeShapeCache` variable** or use it. If unused, deleting it eliminates misleading maintenance surface. If used, flag reference-carrying value types (`RefFields > 0`). | Medium — code quality / future-proofing | Low | High | Improvement |

#### P2 — Medium

| # | Recommendation | Impact | Difficulty | Confidence | Class |
|---|---|---|---|---|---|
| P2-1 | **Expose `AvgBoxedInstanceBytes`** (`TotalBoxedBytes / TotalBoxedObjects`) in domain result and report. Guides remediation strategy (pool small objects vs. restructure large ones). | Medium | Low | High | Improvement |
| P2-2 | **Pre-compute waste before `paddingCandidates.Sort`** to avoid arithmetic in the comparator. Minor performance win; improves readability. | Low | Low | High | Improvement |
| P2-3 | **Rename `OversizedValueTypeCount` to `OversizedValueTypeInstanceCount`** or change semantics to count distinct types. Current name is misleading (counts instances). | Low — UX | Low | High | Improvement |
| P2-4 | **Escalate severity to `Critical` for extreme boxing** (e.g., > 1M boxed objects consuming > 5% of total heap). Currently the maximum finding severity for boxing is `Warning`. | Medium — triage signal | Low | High | Improvement |
| P2-5 | **Extend `BoxedTypeEntry` with `HasReferenceFields` flag** using `TypeShapeEntry.RefFields > 0` (already in index). Reference-carrying boxes are more expensive; this guides class-vs-struct refactoring decisions. | Medium | Low | High | Improvement |

#### P3 — Low

| # | Recommendation | Impact | Difficulty | Confidence | Class |
|---|---|---|---|---|---|
| P3-1 | **Add `IEquatable<T>` absence flag to top boxed types.** Value types missing `IEquatable<T>` box during dictionary equality comparison. Detectable via `clrType.Interfaces`. | Medium | Medium | Medium | Improvement |
| P3-2 | **Add progress reporting** at the ClrMD metadata lookup loop for `TypeScanCap` = Full (50 K iterations). | Low | Low | High | Improvement |
| P3-3 | **Extend index builder to capture per-MT GC generation distribution.** Would enable distinguishing transient (Gen0) from retained (Gen2) boxing — the most impactful missing dimension. | High long-term | High | Medium | Evolution |
| P3-4 | **Add unit tests with synthetic ClrType stubs** covering: enum detection, `Nullable<T>` detection, padding computation, overflow-safe accumulation, cap triggering. Currently only a disk-vs-memory discrepancy integration test exists. | Medium — quality | Medium | High | Improvement |
