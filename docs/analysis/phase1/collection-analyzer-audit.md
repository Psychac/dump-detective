...
| P2-3 | **Per-collection resize recommendation** — add a `Recommendation` field to `WastefulCollectionSnapshot` (e.g., "Call TrimExcess()", "Construct with initial capacity X", "Unreachable — no fix needed"). Populate in the finding generator. | Actionability for engineers | Low | Medium | Improvement | ✅ DONE |
| P2-4 | **Seal `CollectionAnalyzer`** and remove `public void Dispose() { }` override (redundant with default interface implementation). Override `Tags` = `["collections"]` and `Order` = `240`. | Code hygiene, prevents accidental subclass breaking `CreateWorkerInstance` | Low | High | Improvement | ✅ DONE |
...
> **P2-3 note (2026-08-28):** implemented as `CollectionAnalyzer.BuildResizeRecommendation`, called at snapshot-construction time (not the finding generator, which only emits one aggregate finding per analyzer and has no per-collection context) and surfaced as a new "Recommendation" column on the existing "Wasteful collections" table. Deliberately dropped the audit's "Unreachable — no fix needed" case: `RootDescription`'s negative result is `"No root path found (within budget)"` from a budget-limited BFS (`PopulateRootDescriptions`), not proof of unreachability — asserting "no fix needed" off that would be a confidently wrong recommendation. `BuildResizeRecommendation` always returns a concrete capacity fix (kind-specific API — `TrimExcess()`, `ArrayList.TrimToSize()`, or `ImmutableArrayBuilder.ToImmutable()` — or a right-sized-constructor suggestion below a sparse-fill-rate threshold) and takes no reachability signal at all.
...
> **P3-3 note (2026-08-28):** implemented purely in `CollectionSectionBuilder` — no analyzer/domain-model changes needed, since `WastefulCollectionSnapshot` already carried `Head`/`Tail`/`LargestContiguousFreeSegmentBytes`/`FreeSegmentCount`. The shared "Wasteful collections" table dropped those four columns (they were always "—" for every non-Queue kind); a new "Wasteful queues — buffer layout" sub-table renders them for the Queue subset of the same top-N list, so nothing is lost, just relocated. The sub-table is omitted entirely when there are no wasteful queues in the top-N.
...
| P3-3 | **Separate Queue columns in section builder** — render head/tail/free-segment columns only in a Queue-specific sub-table; remove these columns from the shared wasteful-collections table. | Report readability | Low | High | Improvement | ✅ DONE |
| P3-4 | **"Owner type" via partial reverse index** — for top-5 collections, query the partial reverse reference index to surface the immediate referrer type name (owner hint without full BFS). | Quick ownership signal at low cost | Medium | Low | Evolution | ✅ DONE |
| P3-5 | **Per-element-type wasted memory aggregation** — group top `WastefulCollection` items by `ElementType` and surface total wasted bytes per element type. | Unique diagnostic; actionable for allocation refactoring | Medium | Medium | Improvement | ✅ DONE |

> **P3-5 note (2026-08-28):** implemented as exact scan-time accumulation, not a "group the top-N" aggregation as literally worded — grouping only the already-capped top-`N` wasteful list would reproduce the exact P0-1/P2-1 undercounting bug (every element type below the cap would silently vanish from the breakdown). `ElementType` is free-form (unbounded cardinality) unlike `CollectionKind`'s small fixed enum, so it uses `Dictionary<string, int>`/`Dictionary<string, ulong>` accumulators instead of the array-indexed pattern — still only touched from the already-low-frequency `RecordWasteful`/`TrackWasteful` call sites (bounded by wasteful-collection count, not total heap objects), merged across parallel workers the same way as the per-kind accumulators. Surfaced as a "Wasted memory by element type" report table (capped to the top 15 rows for display only — the underlying aggregation itself is exact and uncapped) and a dominant-element-type note in the aggregate finding's evidence.

> **P3-4 note (2026-08-28):** implemented as a single `IBackwardReferenceProvider.TryGetParents` lookup per top-N item (`options.PathAnalysisTopN`, default 5 — same site `PopulateRootDescriptions` already resolves the reverse-index provider for the BFS), computed independently of whether the BFS root-path search below it succeeds, since it's a single indexed lookup rather than a traversal. Discussed with the user first: the reverse index has no notion of which of an object's possibly-multiple recorded parents is the "real" owner, so rather than silently picking `parents[0]` and presenting it as definitive (the source of this item's own "Low confidence" rating), ambiguity is surfaced explicitly — "3 referrers, e.g. CacheManager" — and a truncated extraction (index fanout cap) is marked as a lower bound ("5+ referrers"). Surfaced as a new "Owner (hint)" column, separate from the existing `RootDescription`/"Root" column since the two signals answer different questions (immediate referrer vs. root reachability).
