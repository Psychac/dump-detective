...
| `IParallelHeapIndexScanParticipant` | ✗ (removed 2026-08-12, `bd146c3`) | Formerly implemented for live-counting incoming references; dropped when the reverse-index-provider path became primary (see Audit Area 5 below) — see git history for the old implementation |
...
**2026-09-03 update**: this section predates the 2026-08-12 reverse-index migration (`bd146c3`) and
described a `IParallelHeapIndexScanParticipant`-based reference-counting pass that no longer exists.
`DominatorAnalyzer.AnalyzeAsync` now prefers `BuildLeakSignalsFromReverseIndex` — a single
sequential read of the disk-backed reverse-edge index built once during Phase 1, no ClrMD field
walks, no parallel dispatch, no per-worker accumulator — whenever a reverse index is available.
`AnalyzeObjectsPass` (the assessment below) only runs as a fallback (in-memory mode,
`DD_SKIP_REVERSE_INDEX_BUILD=1`, or a pre-v4 cache.bin), and even there runs as a single, non-
parallel pass — the "K workers" scenario this section originally worried about does not occur.
Kept below for historical accuracy of the fallback path's own characteristics; the K×memory
multiplication callout has been removed as resolved-by-architecture (see the P3 roadmap row).

| Concern | Assessment |
|---|---|
| Reference-counting pass (primary) | `BuildLeakSignalsFromReverseIndex` — single sequential disk read, exhaustive, no ClrMD field walks |
| Reference-counting pass (fallback) | `AnalyzeObjectsPass`, bounded by `MaxLeakScanObjects`; single-threaded, no parallel dispatch |
| `SpaceSavingCounter<ulong>` (fallback only) | Bounded by `MaxReferenceAddresses` (1 M default); acceptable — replaced the old fixed-capacity dictionary in the P3-1 fix (2026-08-27), order-independent |
...
- `SpaceSavingCounter<ulong>` (fallback path only): 1 M entries capacity × (8 + 4 + 8) bytes ≈
  20 MB; acceptable. One instance per analysis run — not per worker, since `AnalyzeObjectsPass`
  is single-threaded (see 2026-09-03 update above).
...
The scalability limit is not the algorithm but the scan cap, and only on the fallback path — the
primary reverse-index path is exhaustive, not sampled. On the fallback path, at a 2 M object limit,
a 100 GB dump with 200 M objects is sampled at 1%. Increasing `MaxLeakScanObjects` there requires
proportionally more RAM, but only for the single `SpaceSavingCounter` instance, not multiplied
across workers.
...
3. **Reference-count dictionary admission ordering affects results** — `AccumulateReference`
   ...
   **2026-08-27 update**: since this audit, `AnalyzeAsync` now branches on
   `context.Cache.TryGetReverseIndexProvider()` — when the disk-backed reverse index
   ([[project_full-reverse-index]]) is available, `BuildLeakSignalsFromReverseIndex` is used
   instead, which enumerates exact child counts via `reverseIndex.EnumerateChildCounts` with
   **no** `MaxReferenceAddresses` cap (see comment at `AnalyzeObjectsPass`'s call site, lines
   ~234-238: "no addresses are ever skipped"). `AccumulateReference`/`CountIncomingReferencesByAddress`
   now only run in `AnalyzeObjectsPass`, the no-reverse-index fallback path. Per
   `ReverseIndexCache.TryGetProvider`'s doc comment this triggers on: in-memory mode (no on-disk
   `cache.bin` at all), `DD_SKIP_REVERSE_INDEX_BUILD=1`, a pre-v4 `cache.bin`, or the reverse-index
   section specifically failing to write during Phase 1 — **not** dump size. A large dump run in
   in-memory mode, against a stale cache.bin, or with the skip env var set would still hit the
   capped path. Blast radius is reduced (opt-in/failure/legacy-cache cases only, not "small dumps
   only") but not eliminated — see revised P3 item below.
...
| Admission ordering bias in ref-count dictionary | ✅ Fixed 2026-08-27 | Replaced by `SpaceSavingCounter<ulong>` in `AnalyzeObjectsPass` — order-independent admission, no more first-come-first-served exclusion; see P3 roadmap item below |
...
**Major Strengths**:
- Primary reference-counting path (`BuildLeakSignalsFromReverseIndex`) is exhaustive and O(1)-per-lookup against a disk-backed index built once during Phase 1 — no live ClrMD scan needed at all in the common case (2026-08-12 migration; superseded the older `IParallelHeapIndexScanParticipant` approach this bullet used to credit).
- Correctly bounds memory usage at every phase (reference-count structure, BFS walk, top-K set).
...
| P3 | Address reference-count admission ordering bias in `AnalyzeObjectsPass`/`AccumulateReference` fallback path only — **2026-08-27: rescoped** (`BuildLeakSignalsFromReverseIndex`, the primary disk-index path, already has no cap and is unaffected), then **✅ DONE same day**: replaced the fixed-capacity `Dictionary<ulong,int>` with a new generic, reusable single-pass Space-Saving heavy-hitters counter (`DumpDetective.Analysis.Algorithms.SpaceSavingCounter<TKey>`, `docs`-referenced from its own XML doc comment) — order-independent by construction (any address whose true frequency exceeds `n / capacity` is guaranteed to survive, per Metwally/Agrawal/El Abbadi 2005), single heap pass (no doubled ClrMD field-read cost, unlike the two-pass option considered and rejected), bounded O(capacity) memory identical to the old dictionary. `LeakSignals`/`DominatorDomainResult`'s `SkippedReferenceAddresses` renamed to `ApproximatedReferenceAddresses` to reflect the new bounded-error-not-dropped semantics; report/finding text updated to match. Unit-tested in `tests/DumpDetective.Tests/Unit/Algorithms/SpaceSavingCounterTests.cs`, including a regression test for the exact late-arriving-high-frequency-key scenario this fixes. | Low (was Medium) — only hit when reverse index unavailable | High | Medium | Improvement | ✅ DONE |
...
| P3 | Dominator chain detection (A → B → C with cumulative retained bytes) — **2026-08-27: rescoped** (audit predates the exact Lengauer-Tarjan tree shipping 2026-08-16; `IDominatorTreeProvider.TryGetImmediateDominator`/`TryGetRetainedBytes` already give an O(log n) mmap'd parent pointer + retained bytes per hop, so this is a walk over existing disk data, not new computation), then **✅ DONE same day**: new `DominatorAnalyzer.BuildDominatorChain` (internal, unit-tested) walks each Gen2/LOH candidate's sample address up to the virtual root, bounded by new `RetentionOptions.MaxDominatorChainDepth` (safety cap only, no separate display truncation — see remarks on that option for why an uncapped walk isn't safe: a linked-list-shaped heap can make dominator-chain depth unbounded). Result exposed as `DominatorDomainResult.DominatorChainsByTypeName`. Rendered via the shared collapsible tree widget (see line below and `docs/refactor/collapsible-tree-widget-design.md`'s adoption tracking) instead of a narrative text block — each hop is its own `TreeNode` (not string-joined) since retained bytes differ per hop and are worth surfacing individually; `IsChain: true` throughout. Tests: `tests/DumpDetective.Tests/Unit/Analyzers/DominatorAnalyzerChainTests.cs` (walk termination, ordering, sentinel-on-cap) and `DominatorSectionBuilderTests` (tree-widget shape, Gen2/LOH-only scoping). | High — root cause identification | High | Medium | Evolution | ✅ DONE |
| P3 | Cross-type retained-overlap metric ("shared subgraph size") | Medium — explains why exclusive retained bytes are 0 for co-dominating types | High | Medium | Evolution | **2026-09-03: ✅ DONE (§8a/§8b/§8c, all three)** — see [dominator-tree-phase1-integration.md §8](../phase1-redesigns/dominator-tree-phase1-integration.md#8-cross-type-retained-overlap-shared-subgraph-size). §8a: `DominatorAnalyzer.FindContainingCandidateTypeName` finds, for each Gen2/LOH candidate, the nearest other candidate whose sampled subtree fully contains it (free — reuses the existing dominance-chain walk), rendered as "Shared subgraph overlap (sample-based)". §8b/§8c (one combined change): `ComputeCrossTypeOverlap` streams the existing disk-backed heap index for every instance of every candidate type (safety-capped via new `RetentionOptions.MaxCrossTypeOverlapInstancesScanned`, honestly flagged if hit); `WalkInstanceAncestry` counts how many instances of one type are nested inside another's subgraph (§8b) and sums exact retained bytes over each pair's "topmost" instances only, avoiding the same double-counting §5's fix addresses (§8c) — both rendered together in the "Cross-type retained overlap" table (Instances + Retained columns). Same investigation also found and fixed an unrelated correctness bug: the exact per-type "Retained" column double-counted same-typed nesting (e.g. linked lists) — see §5 addendum in the same doc, dated 2026-09-03 |
| P3 | Cap or disk-spill per-worker `_referenceCount` in parallel mode (see design sketch follow-up) | Medium — bounds K × 20 MB peak RSS at high parallelism | Medium | Medium | Improvement | **2026-09-03: ✅ RESOLVED BY ARCHITECTURE, no new work needed** — the `K × 20 MB` premise assumed `DominatorAnalyzer` ran as an `IParallelHeapIndexScanParticipant` with one `_referenceCount` accumulator per worker. That interface was dropped entirely on 2026-08-12 (`bd146c3`) when the disk-backed reverse-index path became primary; the old per-worker accumulator concern no longer applies — see Audit Area 5's 2026-09-03 update in the same doc. The remaining fallback path (`AnalyzeObjectsPass`, in-memory/`DD_SKIP_REVERSE_INDEX_BUILD=1`/pre-v4-cache only) holds exactly one `SpaceSavingCounter` instance, single-threaded, never multiplied. |
| P2 | Render an inline root-chain summary (kind + capped hop list) per row in the Gen2/LOH dominator sub-table, reusing `RootPathFinder`/`RootPathGroup` already used by `GCRootIntelligenceSectionBuilder` | High — closes the report-clarity gap vs. comparable tools (Audit Area 8 item 1); today an engineer must cross-reference a separate section by hand | Medium — `PathTypeNames` is target-outward today, not root-to-target; needs a direct `RootPathFinder` call against the sample address, not just reuse of existing evidence | Medium | Improvement | **2026-09-03: ✅ DONE** — `DominatorAnalyzer.ComputeRootChains` calls `RootPathFinder.TryFindAnyRootPath` once per Gen2/LOH candidate (same construction pattern as the analyzer's existing `PopulateEvidence` "highly referenced objects" evidence search — `ReferenceGraph` + `RootPathSearchSupport`'s shared no-op telemetry/noise predicate + the `RetentionOptions.MaxRootPathCandidateNodes`-family limits, which turned out to already exist unused, seemingly added in anticipation of exactly this item), resolved into a root-most-first hop-type-name list (`RootChainSummary`). Rendered as a 4th adopter of the shared collapsible tree widget — "Gen2 / LOH root paths," alongside the existing "Gen2 / LOH dominance chains" (P3-3) — complementing it rather than replacing it: dominance chain answers "who retains this," root chain answers "why is this alive." Deliberately scoped to just the Gen2/LOH candidate set, not the full top-K table, since a `RootPathFinder` search is a real bidirectional BFS per candidate, unlike the free point-lookups everything else on this path uses. |
| P2 | Add narrative interpretation text for the retained/shallow ratio (e.g. "retained >> shallow → holds an external graph" vs. "retained ≈ shallow → self-contained") next to the Gen2/LOH sub-table — **2026-09-03: scoped up** to a shared reporting-layer convention rather than a Dominator-only text block, since several other analyzers emit bare ratio/percent metrics with the same gap; see [narrative-interpretation-text-design.md](../../refactor/narrative-interpretation-text-design.md) (this item is its first adopter) | Medium — turns an uninterpreted number into an actionable read, negligible cost (no new data) | Trivial | High | Improvement | **2026-09-03: ✅ DONE (scoped to this audit)** — new `InterpretationBlock` (`AnalyzerDetailSection.cs`) + `SectionBuilderBase.Interpret(value, tiers...)` helper, rendered by all three report formatters (Markdown, HTML, plain text). `DominatorSectionBuilder` interprets the top-ranked Gen2/LOH candidate's retained/shallow ratio (the sub-table is already sorted by Gen2Count/LohBytes descending), naming the type so the blurb is unambiguous about which row it describes. The other candidate adopters listed in the design doc (`StringSectionBuilder`, `WeakReferenceSectionBuilder`, `LohFragmentationSectionBuilder`, `GCPressureSectionBuilder`) remain deliberately unadopted per that doc's own "don't force items 2–3 into the same change as item 1" rule — out of scope for this audit. |
| P3 | Dedup rendered root chains by identical shape with a "×N instances — same chain" marker, once item 1 above exists | Medium — prevents the improved chain rendering from becoming noisier than today's bare-address baseline at scale | Low (once item 1 lands) | High | Improvement | **2026-09-03: ✅ DONE** — `DominatorSectionBuilder` groups Gen2/LOH candidates by chain shape (root kind + every hop except the last, since the last hop is always the candidate's own type and differs by construction) before rendering the "Gen2 / LOH root paths" tree widget, so N candidates sharing an ancestor path collapse into one tree with a "×N types — same chain: TypeA, TypeB, …" leaf (capped at 5 names shown, "+N more" beyond that) instead of N near-identical trees. A chain with no ancestor hops at all (the candidate is itself a direct GC root) is deliberately never grouped, even with another candidate rooted the same way — sharing only a root-kind label isn't a shared path. Unit-scoped to per-type sample chains (the "×N" counts distinct Gen2/LOH *candidate rows*, not raw object instances — a different unit than §8b's population counts). Tests: `DominatorSectionBuilderTests` (dedup into one tree, distinct shapes stay separate, direct-root chains never grouped, name-list capping). |
| P3 | Shared "Next steps" `SectionBlock` convention pointing from this section to `GCRootAnalyzer`/`ReferenceChainAnalyzer` output, matching the other tool's step-numbered investigation flow | Medium — reporting-layer change, not analyzer-specific; benefits every analyzer's section, not just this one | Medium | Medium | Evolution | **2026-09-03: ✅ DONE, full fix (not the plain-text-only shortcut)** — new `NextStepsBlock`/`NextStepLink` (`AnalyzerDetailSection.cs`) + `SectionBuilderBase.NextSteps(params (Label, AnalyzerName)[])`, resolving each target's stable `SectionId` via `SectionIdDomainMap` and silently omitting anything unresolvable (never a dead-on-arrival link). Two real gaps had to close first, not just the block plumbing: (1) `ReportHtmlShared.RenderAnalyzerSections` (the server-side pre-render path for very large reports) anchored sections by array index only, ignoring `SectionId` entirely, unlike `MarkdownCanonicalReportFormatter` which already preferred it — fixed to match, same `SectionId`-with-index-fallback scheme (the primary client-side JS renderer, `report.renderers.sections.js`, already handled this correctly via a legacy-alias anchor, so this only affected the C# fallback path); (2) `ReferenceChainAnalyzer` had no `SectionId` at all (`("Memory", "")`) — assigned the previously-unused `"A4"` slot, since it sits conceptually next to `GCRootAnalyzer` in Domain A. `DominatorSectionBuilder` now emits a "Next steps" block pointing at both. Rendered as real hyperlinks in Markdown/HTML, a section reference in plain text. Tests: `SectionBuilderBaseNextStepsTests` (resolution, unresolvable-target skipping, ordering), `ReportHtmlSharedAnchorTests` (SectionId-based anchor, index fallback), `DominatorSectionBuilderTests` (wiring). Other analyzer sections can adopt the same helper opportunistically — not scoped further here. |
| P3 | Render the existing dominator parent/child structure (`DominatorRetainedSetAggregator`) as a browsable tree using the shared collapsible tree widget (once built for Thread Stack Cluster P3-2), instead of only a flat top-N table — **✅ DONE 2026-08-27** as part of the dominator-chain-detection item above: `DominatorSectionBuilder` now emits a `TreeWidget` per Gen2/LOH candidate (its dominance chain), the widget's third planned adopter per `docs/refactor/collapsible-tree-widget-design.md`. Scoped to the per-type ancestor chain, not a full browsable multi-branch dominator tree (that would need merging chains across candidates into a shared trie, à la `RootPathGroups`/Thread Stack Cluster — left as a further follow-up if the per-row chain proves not enough) | High — lets engineers walk the actual dominator hierarchy interactively, closer to dotMemory/VS diagnostics | Medium (depends on shared widget) | Medium | Evolution | ✅ DONE (partial — per-row chain, not merged tree) |
