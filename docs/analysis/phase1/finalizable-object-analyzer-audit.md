...
- InsightEngine houses finalizer-related correlation rules (`DetectFinalizerQueueBacklog`, `DetectEventLeakPattern`, `DetectDataTableLifecyclePattern`) that cross-reference thread state and GC generation data. (`DetectKnownFinalizerQueuePatterns` was single-analyzer-only and has since been moved into `FinalizableObjectFindingGenerator` — see item 6 below.)
...
6. **✅ FIXED: `DetectKnownFinalizerQueuePatterns` lived in InsightEngine** rather than being surfaced through the analyzer's own finding generator, making the heuristics (DynamicResolver, Thread abandonment, TimerHolder, ReaderWriterLock) invisible unless InsightEngine ran. Moved into `FinalizableObjectFindingGenerator` and now matched against the real finalizer queue (`TopQueueTypesByCount`) instead of the Gen2 population sweep it was previously (incorrectly) reading.
...
| ✅ DONE | Detect CriticalFinalizerObject / SafeHandle accumulation — check `ClrType` hierarchy or type name | High (diagnostic) | Medium | High | Improvement |
...
| ✅ DONE | Move `DetectKnownFinalizerQueuePatterns` heuristics into `FinalizableObjectFindingGenerator` — also fixed a semantic bug in the process: the heuristics matched on `TopFinalizableTypesByGen2Count` (population sweep) while claiming to describe the finalizer queue; now matched against `TopQueueTypesByCount` (the real queue) with re-tuned thresholds. Old InsightEngine method deleted. | Medium (UX) | Medium | High | Improvement |
| ✅ SUPERSEDED | ~~Reservoir-sample queue entries instead of first-N to improve retained-size coverage~~ — moot: `b2e8cf1` removed all queue caps (`QueueScanLimit`/`TopQueueEntries`); every queue entry is now processed exhaustively with exact dominator-tree retained bytes, which is strictly better coverage than any sampling scheme could offer | Medium (correctness) | Medium | High | Improvement |
...
| ✅ SUPERSEDED | ~~Replace full `finalizableTypes.Sort()` with top-K partial sort when count >> TopTypeLimit~~ — premise gone: `b2e8cf1` removed `TopTypeLimit`/`options.TopTypeLimit` entirely, and `TopFinalizableTypesByGen2Count` (like the queue lists) now intentionally carries the full, exact, untruncated type list rather than a top-K sample, matching the project-wide move away from biased "top N" sampling. `finalizableTypes.Sort()` is O(n log n) over distinct type count (bounded, cheap), not object count — a partial-K sort would only reintroduce the truncation this codebase has been deliberately removing. Display-side truncation (`RowLimit`) already happens client-side in `report.renderers.sections.js`, not server-side. | Low (perf) | Easy | Medium | Improvement |
| ✅ SUPERSEDED | ~~Reuse `HashSet<ulong>` + `Queue` across BFS calls via `ArrayPool`~~ — moot: `b2e8cf1` deleted the bounded-BFS retained-size estimator (`BfsEstimateRetained`) and `BoundedGraphWalk.cs` entirely; retained bytes now come from `IDominatorTreeProvider.TryGetRetainedBytes`, a pre-built index lookup with no per-entry BFS traversal, so there is no `HashSet`/`Queue` allocation left to pool. | Low (perf) | Medium | High | Improvement |
| ✅ DONE | Add per-queue-entry generation field (`Gen0/1/2/LOH`) to `FinalizerQueueEntry` — added `Generation` field, populated via `SegmentKindMapper.ResolveGeneration` per entry, displayed as a "Generation" column (Gen 0/1/2/LOH/Unknown) in the section table. | Medium (diagnostic) | Easy | High | Improvement |
| ✅ DONE | Expose root path cross-reference for top queue entries via `RootIndexReader` — added `SampleRootPath`/`RootPathSearchTruncated` to `FinalizerQueueEntry`, populated for the top 10 entries by retained size via `RootPathFinder` (same pattern as `TimerLeakAnalyzer.PopulateEvidence`); every entry still keeps exact count/bytes/generation, only this expensive per-object path search is bounded to the highest-impact entries. Surfaced as a "Retention evidence" block in the section, matching `TimerLeakSectionBuilder`. | High (diagnostic) | Hard | Medium | Evolution |
...
| ✅ SUPERSEDED | ~~Add InsightEngine rule: queue count delta growing across trend series → dedicated finding~~ — doesn't fit InsightEngine's architecture (`Analyze(IReadOnlyList<AnalyzerRunResult> runs)` only ever sees a single run, by design — no baseline/trend data reaches it) and is already redundant: `MetricDelta.Classification` (Regression/SevereRegression/Improvement) plus the generic §T4 `TrendMetricTimelineSectionBuilder` already surfaces per-analyzer regression counts and timelines for every `IAnalyzerTrendComparer` metric, including `finalizable.queue.count` (`FinalizableObjectTrendComparer`, `HigherIsWorse`) — no finalizer-specific work needed. | Medium (diagnostic) | Medium | Medium | Evolution |
...
3. **Platform evolution opportunities:** ✅ DONE (2026-08-25): Root path cross-reference for queue entries via `RootIndexReader`/`RootPathFinder` — the top 10 queue entries by retained size now get a "why is it alive" GC root path, converting the section from "how much memory" to "why is it alive". This was the last open roadmap item.

4. **Highest engineering return:** P0+P1 items complete (~2–3 hours combined). Score improved from 68 → 82/100. **Roadmap status (2026-08-25): all P2/P3 items are now closed** — CriticalFinalizerObject/SafeHandle detection, known-queue-pattern findings (relocated from InsightEngine with a semantic fix), per-entry generation field, and root-path cross-reference all shipped; reservoir-sampling, partial top-K sort, BFS-buffer pooling, and an InsightEngine trend-delta rule were all found superseded by prior architectural changes (exact/full-data dominator-tree integration, and the generic §T4 trend timeline) and closed without new code. See the Priority Roadmap table above for per-item detail.
