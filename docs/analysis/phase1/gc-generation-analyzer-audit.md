...
| P1-2 | **Add POH detection.** Check `ClrSegment.Kind` for `GCSegmentKind.Pinned`; add `PohBytes`/`PohObjects` to `GCGenerationDomainResult`. LOH and POH are different heaps on .NET 5+ and conflating them is a diagnostic error. | High — correctness on .NET 5+ | Medium | High | Improvement | ✅ DONE (commit 1284feb) |
| P1-3 | **Add LOH fragmentation ratio.** Walk LOH segments for free list objects; compute `LohFreeBytes / LohCommittedBytes`. Add to domain result and section. | High — LOH fragmentation is a leading cause of OOM | Medium | High | Improvement | ⏭️ SKIPPED — Delegated to `LohFragmentationAnalyzer` (dedicated specialist analyzer) |
...
| P3-2 | **Remove the public `Analyze(ClrHeap, IHeapAnalysisCache)` overload.** It bypasses progress reporting and exists only for the test path. Replace with a test-scoped helper. | Low — cleanup | Low | Medium | Improvement | ✅ DONE — overload had zero call sites (not even in tests); deleted outright, no replacement helper needed |
| P3-3 | **Add a GC segment map diagnostic** showing segment count, committed memory, and ephemeral segment utilization. | Medium | High | Medium | Evolution | ⏭️ SKIPPED — `HeapTopologyAnalyzer`/`HeapTopologyDomainResult` already provides segment count, committed/reserved/used bytes, per-kind and per-logical-heap breakdowns, and per-segment Gen0/1/2 split; a second implementation here would duplicate it. **Found + fixed a real bug while scoping this**: `AnalyzerHelpers.ComputeExactGenBytes` (used by this analyzer and `AllocationPatternAnalyzer`) attributed a classic (non-regions) `Ephemeral` segment's *entire* committed size to Gen2 as a "conservative" fallback, zeroing out Gen0/Gen1 bytes on every classic-GC dump. Now uses `segment.Generation0/1/2` sub-range lengths — the same accurate per-segment split `SegmentSummaryCache` already used — for every SOH-kind segment. |
| P3-4 | **Emit finalizable-in-Gen2 count as a cross-reference** to `FinalizableObjectAnalyzer`. This connects a signal that currently exists silently in the profiles. | Low | Low | High | Evolution | ✅ DONE — `GCGenerationDomainResult` gained `FinalizableGen2Count`/`FinalizableGen2Bytes`, summed from `TypeGenerationProfile.IsFinalizable` types while `GCGenerationAnalyzer` already builds the per-type profiles (no extra heap pass); surfaced as key metrics + narrative in `GCPressureSectionBuilder` and folded into the Gen2-pressure finding's evidence in `GCGenerationFindingGenerator`, both pointing readers at `FinalizableObjectAnalyzer` for finalizer-queue detail |
| P3-5 | **Optimize `genCandidates` list.** Reuse the already-populated list instead of building a full copy of all aggregates for the gen profile sort. | Negligible in practice | Low | High | Improvement | ✅ DONE — `genCandidates` and `lohCandidates` now presized to `aggregates.Count` (exact size and safe upper bound, respectively), avoiding `List<T>`'s doubling-growth reallocations/copies for heaps with hundreds of thousands of distinct types |
