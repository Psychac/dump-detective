...
| 18 | **WeakReferenceAnalyzer** | 2/2 | 4/4 | 5/5 | 4/4 | ✅ P0+P1+P2+P3 COMPLETE — P3-2 shipped via `IReachableAddressProvider` (Stage A walk) rather than the raw reverse-edge index, avoiding a static-root false-positive the naive approach would have hit; P3-4 upgraded from "label the estimate" to "make the index path exact" via a disk-index-filtered streaming scan (same convention as AsyncStateMachineAnalyzer/TimerLeakAnalyzer/EventLeak), with the old sample-extrapolation kept only as a should-not-occur-in-production degraded fallback. No dedicated analyzer-level unit test yet for the new scan path (no existing WeakReferenceAnalyzer test fixture) |
...
| 20 | **HttpObjectAnalyzer** | 2/2 | 3/3 | 5/5 | 3/3 | ✅ P0+P1+P2+P3 COMPLETE (2026-08-31) — P2: per-instance `HttpClient`/`HttpWebRequest` sampling unified under one `ITypedResourceInstanceSampler<HttpInstanceSnapshot>` (URL, timeout, response-pending — cross-runtime, field names verified by decompiling both .NET Framework `System.dll` and .NET 9 `System.Net.Requests.dll` rather than assumed), `IHttpClientFactory` handler-rotation detection (`ActiveHandlerTrackingEntry`/`ExpiredHandlerTrackingEntry`, corrected the audit's wrong nested-type assumption via decompile), free GC-generation breakdown for `HttpClient` via the already-populated `HeapEntry.Generation` field (no new ClrMD calls), `long`-widened per-category counters matching the project's existing `Gen0Count`/`Gen1Count`/`Gen2Count` precedent, and a handler-per-client ratio plus HttpMessageHandler-by-module breakdown (reusing `TypeAggregateNameResolver.ResolveModuleName`) — P2-5's literal per-instance handler chain-depth walk deliberately excluded as disproportionate to the module breakdown already shipped. P3: `HttpMessageHandler` accumulation finding (closing the last HTTP category with no finding coverage) and `ServicePoint.ConnectionLimit` sampling (again cross-runtime field-name drift, `_connectionLimit` vs `m_ConnectionLimit`, confirmed by decompile) both shipped; P3-3 (register as `IHeapIndexScanParticipant`) was already done pre-dating this session. See `http-object-analyzer-audit.md` |
...
> above.
>
> **WeakReferenceAnalyzer resolved:** P3-2 ("held only via weak reference" detection) shipped —
> implemented against `IReachableAddressProvider` (Stage A's reachability walk) rather than a raw
> `TryGetParents` join, to avoid a static-root false-positive the naive approach would have hit.
> See `weak-reference-analyzer-audit.md`. Not moved to "already wired" above since it doesn't use
> `RootPathFinder`/`TryGetParents` directly.
