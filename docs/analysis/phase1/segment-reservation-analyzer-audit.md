...
|---|---------------|--------|-----------|-----------|---------------|
| **P0** | Fix `IntPtr.Size` → use `context.Runtime.DataTarget.DataReader.PointerSize` for dump bitness detection in both analyzer and finding generator | High — silent correctness defect for 32-bit dumps | Low | Very high | Improvement |
