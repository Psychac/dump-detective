...
| **P3** | Add `segment.End` address to `SegmentReservationEntry` for parity with `!eeheap -gc` | Low | Low | High | Improvement | ✅ DONE |
| **P3** | Add `IsServer` flag and logical heap count to domain result | Low — contextualises multi-heap data | Low | High | Improvement | ✅ DONE |
| **P3** | Investigate `ClrSegment.IsEphemeral` in the current ClrMD version and prefer it over custom detection | Low | Low | Medium | Improvement | ✅ DONE — no such property exists in ClrMD 4.0.732401 (`ClrSegment` members: `Address, ObjectRange, Start, End, Length, SubHeap, ReservedMemory, CommittedMemory, FirstObjectAddress, Kind, IsPinned, Generation0/1/2, EnumerateObjects, GetGeneration`). `SegmentKindMapper.IsEphemeral` already uses an exhaustive `switch` on `GCSegmentKind` (fixed under P2, cd1c80f) with a documented `Generation0.Length > 0` fallback for regions-based heaps — this remains the correct approach for this ClrMD version. No code change required. |
| **P3** | Explore regions-based GC per-region statistics for .NET 8+ dumps | High long-term | High | Medium | Evolution (new capability) | ✅ DONE — regions-mode detection (`IsRegionsBased`, keyed off `GCSegmentKind.Generation0`/`Generation1` presence, per ClrMD doc: only emitted for regions-based GC), per-generation region bucket stats (`RegionGenerationStats`: count, total/min/max/avg reserved, near-empty count and committed bytes), fill % extended to all segment kinds when regions-based (previously ephemeral-only), new "Regions by generation" table, near-empty-region decommit-candidate finding, and two new trend metrics. **Caveat**: no regions-based (.NET 8+) dump or synthetic-heap test harness was available to validate against — classic (non-regions) dumps are unaffected (verified: `IsRegionsBased` stays `false`, existing metrics/behavior unchanged), but the regions-mode path is unverified against a real regions-based dump. |
