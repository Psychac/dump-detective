...
| P0-3 | **Propagate BFS-cap flag** — when `CollectRetainedObjects` hits `maxObjects`, set a flag on `StaticRootAnalysis` and surface `"(size estimate — scan capped)"` in the report | Correctness/Confidence | Low | High | Improvement | Pending |
