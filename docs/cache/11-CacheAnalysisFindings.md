...
| Item | Fix |
|---|---|
| `CrashAnalyzer` `InferredTraceCount` mismatch (disk=1, memory=0) | `RunParallelExceptionScan` collected exception instances into a `ConcurrentBag` (thread-scheduler-dependent order), then capped each type's list at `MaxExceptionsPerType`; which instances survived the cap determined whether Tier 2-4 stack-trace inference in `BuildCrashThreadSnapshotsImpl` had a matching candidate to find. Fixed by sorting the collected instances by address before capping, matching the deterministic ascending-address order `RunSequentialExceptionScan` (disk mode) already used. |
...
| **Finding 7** — `BoxingAnalyzer` `TotalBoxedObjects` off-by-45 | `TypeScanCap` capped a raw `foreach` over `HeapIndexBuildResult.TypeAggregates`, a `Dictionary<ulong, TypeAggregateIndexEntry>` whose enumeration order is an artifact of parallel segment-merge insertion order — non-deterministic and different between disk and memory index builders. Above the 10,000-type default cap, each mode truncated to a different arbitrary subset of types, so every aggregate field diverged. Fixed by sorting entries by `TotalSize` desc (`MethodTable` tiebreak) before capping, only when `typeAggregates.Count > TypeScanCap` so the common case pays no extra cost. `BoxingAnalyzerDiscrepancyTests` now passes. |
...
| Finding 7 — `BoxingAnalyzer` `TotalBoxedObjects` off-by-45 | **Fixed** | `TypeScanCap` truncation iterated `TypeAggregates` (a `Dictionary`) in raw insertion order, which differs between disk/memory index builders' parallel segment-merge order. Above the 10,000-type cap this truncated to a different arbitrary subset of types per mode. Now sorts by `TotalSize` desc (MethodTable tiebreak) before capping, only when `Count > TypeScanCap`. `BoxingAnalyzerDiscrepancyTests` passes. |
