...
| **True dominator tree (exact retained bytes)** | **We have this; they don't.** `LengauerTarjan.cs`/`DominatorTreeComputer.cs` (this branch) implement a real Lengauer-Tarjan dominator-tree algorithm over the whole reachable heap, wired into `DominatorAnalyzer`/`DominatorSectionBuilder`'s Gen2/LOH sub-table as exact retained bytes when the exact path succeeds, falling back to the pre-existing bounded-BFS heuristic otherwise (`RetentionOptions.EnableExactDominatorTree`, default on). Confirmed by direct source read plus a whole-repo text search on the other side: `grep -rli "dominator\|idom\|lengauer"` across every `.cs` file in `Rohit_DumpDetective` returns **zero matches**. | Confirmed absent. Their "retained size" (`SuspectRow.RetainedSize`, `BfsIndexCache.ComputeRetained`) is a BFS-based approximation over their 3-pass BFS index — the `(Size, bool Estimated)`/`(Bytes, bool Estimated)` return shapes make explicit that it's a known-approximate estimate, not exact dominance. See [architecture-comparison.md](architecture-comparison.md) § Retained-memory computation for the full comparison and the report-clarity gap this doesn't cover. |
