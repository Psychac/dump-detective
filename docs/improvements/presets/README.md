# Preset Reference — Analyzer Presets

This directory contains per-analyzer preset guidance (`Fast` / `Balanced` / `Full`) that map to the concrete `*Options.Preset(AnalysisProfile)` values in code. Use these docs to choose sensible analysis profiles for different dump sizes.

Preset files (one per analyzer):

- [allocation-pattern-analyzer-presets.md](docs/improvements/presets/allocation-pattern-analyzer-presets.md) — ✅ **COMPLETED**
- [appdomain-analyzer-presets.md](docs/improvements/presets/appdomain-analyzer-presets.md) — ✅ **COMPLETED**
- [array-analyzer-presets.md](docs/improvements/presets/array-analyzer-presets.md)
- [async-state-machine-analyzer-presets.md](docs/improvements/presets/async-state-machine-analyzer-presets.md)
- [async-task-analyzer-presets.md](docs/improvements/presets/async-task-analyzer-presets.md)
- [boxing-analyzer-presets.md](docs/improvements/presets/boxing-analyzer-presets.md)
- [collection-analyzer-presets.md](docs/improvements/presets/collection-analyzer-presets.md)
- [crash-analyzer-presets.md](docs/improvements/presets/crash-analyzer-presets.md)
- [dependent-handle-analyzer-presets.md](docs/improvements/presets/dependent-handle-analyzer-presets.md)
- [event-leak-analyzer-presets.md](docs/improvements/presets/event-leak-analyzer-presets.md)
- [finalizable-object-analyzer-presets.md](docs/improvements/presets/finalizable-object-analyzer-presets.md)
- [gchandle-analyzer-presets.md](docs/improvements/presets/gchandle-analyzer-presets.md)
- [gcgeneration-analyzer-presets.md](docs/improvements/presets/gcgeneration-analyzer-presets.md)
- [gcroot-analyzer-presets.md](docs/improvements/presets/gcroot-analyzer-presets.md)
- [hang-analyzer-presets.md](docs/improvements/presets/hang-analyzer-presets.md)
- [jit-analyzer-presets.md](docs/improvements/presets/jit-analyzer-presets.md)
- [lock-graph-analyzer-presets.md](docs/improvements/presets/lock-graph-analyzer-presets.md)
- [loh-fragmentation-analyzer-presets.md](docs/improvements/presets/loh-fragmentation-analyzer-presets.md)
- [memory-analyzer-presets.md](docs/improvements/presets/memory-analyzer-presets.md)
- [memory-leak-analyzer-presets.md](docs/improvements/presets/memory-leak-analyzer-presets.md)
- [module-analyzer-presets.md](docs/improvements/presets/module-analyzer-presets.md)
-- [object-shape-analyzer-presets.md](docs/improvements/presets/object-shape-analyzer-presets.md) — ✅ **COMPLETED**
- [reference-chain-analyzer-presets.md](docs/improvements/presets/reference-chain-analyzer-presets.md)
- [segment-analyzer-presets.md](docs/improvements/presets/segment-analyzer-presets.md)
- [segment-reservation-analyzer-presets.md](docs/improvements/presets/segment-reservation-analyzer-presets.md)
- [string-analyzer-presets.md](docs/improvements/presets/string-analyzer-presets.md) — ✅ **COMPLETED**
 - [thread-analyzer-presets.md](docs/improvements/presets/thread-analyzer-presets.md) — ✅ **COMPLETED**
 - [thread-stack-cluster-analyzer-presets.md](docs/improvements/presets/thread-stack-cluster-analyzer-presets.md) — ✅ **COMPLETED**
- [weak-reference-analyzer-presets.md](docs/improvements/presets/weak-reference-analyzer-presets.md) — ✅ **COMPLETED**

If a file is missing or you want entries re-ordered, tell me and I will update this index.
