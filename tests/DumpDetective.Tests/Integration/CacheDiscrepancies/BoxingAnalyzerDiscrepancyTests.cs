...
public sealed class BoxingAnalyzerDiscrepancyTests
{
    ...
    public async Task BoxingAnalyzer_DiskVsMemoryMode_AgreeOnSameHeap()
    {
        ...
        string freshDumpPath = dumpPath + ".freshdiskcheck.BoxingAnalyzerDiscrepancyTests";
        ...
        try
        {
            ...
            diskResult.AvgBoxedInstanceBytes.Should().Be(memResult.AvgBoxedInstanceBytes);
            diskResult.BoxedEnumCount.Should().Be(memResult.BoxedEnumCount);
            diskResult.BoxedEnumBytes.Should().Be(memResult.BoxedEnumBytes);
            diskResult.NullableBoxedCount.Should().Be(memResult.NullableBoxedCount);
            diskResult.NullableBoxedBytes.Should().Be(memResult.NullableBoxedBytes);
            diskResult.OversizedValueTypeInstanceCount.Should().Be(memResult.OversizedValueTypeInstanceCount);
            diskResult.AggregatePaddingWasteBytes.Should().Be(memResult.AggregatePaddingWasteBytes);
            diskResult.TypeScanCapped.Should().Be(memResult.TypeScanCapped);
            diskResult.TypeScanCapUsed.Should().Be(memResult.TypeScanCapUsed);
