...
public sealed class ReportingA11yTests
{
    ...
    {
        var serializer = new DumpDetective.Reporting.Serialization.ReportSerializer();
        ...
        string html = renderer.Render(doc, new HtmlRenderSettings(PreRender: false, StyleVersion: DumpDetective.Reporting.Models.ReportStyleVersion.V2));

        html.Should().Contain("timeline-delta-chip");
        html.Should().Contain("aria-label");
        html.Should().Contain("role=\"img\"").OrContain("role=\'img\'");
    }
